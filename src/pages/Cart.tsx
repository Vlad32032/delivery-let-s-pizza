...
const Cart: FC = () => {
	...
	const { totalPrice, totalCount, cartItems } = useAppSelector((state) => state.cart);
    ...
    const onClickPay = () => {
        switch (clickCount) {
            case 0: 
                alert("Оплатить не получиться 😕, это учебный проект");
                setClickCount(clickCount + 1);
                break;
            case 1:
                alert("Я серьезно, оплатить не получиться 😐, это просто учебный проект");
                setClickCount(clickCount + 1);
                break;
            case 2: 
                alert("Да хватит уже сюда нажимать 😡, это просто учебный проект");
                setClickCount(clickCount + 1);
                break;
            case 3: 
                alert("Ну все, нажать больше не получиться, я убрал кнопку 😎");
                setClickCount(clickCount + 1);
                break;
        }
    }
            ...
            { cartItems.length === 0 ? (
                ...
                <div className="cart">
                    ...
                    <div className="content__items">
                        {
                            cartItems.map((pizza) => <CartItem {...pizza} key={pizza.id}/>)
                        }
                    </div>
